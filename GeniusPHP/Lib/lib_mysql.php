 ...
 * Make sure php_pdo_xxx (xxx = the database name you want to use) extension is enabled and installed on php.ini.
 * A little bit SQL knowledge.
 * On PHP 5.4+, you can use [] as short array syntax. 
 *        
 */
final class Mysql
{
    ...
    public function __construct()
    {
        ...
        try {
            ...
            switch ($type) {
                case 'mysql':
                case 'pgsql':
                    $this->pdo = new PDO($type . ':host=' . $this->server . ';dbname=' . $this->database_name, $this->username, $this->password, $this->option);
                    break;
                case 'mssql':
                case 'sybase':
                    $this->pdo = new PDO($type . ':host=' . $this->server . ';dbname=' . $this->database_name . ',' . $this->username . ',' . $this->password, $this->option);
    ...
    {
        return 0 + ($this->query('SELECT COUNT(*) FROM ' . $table . $this->where_clause($where))
            ->fetchColumn());
    }
    ...
    {
        return 0 + ($this->query('SELECT MAX(' . $column . ') FROM ' . $table . $this->where_clause($where))
            ->fetchColumn());
    }
    ...
    {
        return 0 + ($this->query('SELECT MIN(' . $column . ') FROM ' . $table . $this->where_clause($where))
            ->fetchColumn());
    }
    ...
    {
        return 0 + ($this->query('SELECT AVG(' . $column . ') FROM ' . $table . $this->where_clause($where))
            ->fetchColumn());
    }
    ...
    {
        return 0 + ($this->query('SELECT SUM(' . $column . ') FROM ' . $table . $this->where_clause($where))
            ->fetchColumn());
    }
